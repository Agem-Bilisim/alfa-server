...
/**
 * Credits to http://moi.vonos.net/java/symmetric-encryption-bc/ and
 * https://stackoverflow.com/questions/5641326/256bit-aes-cbc-pkcs5padding-with-bouncy-castle
 * 
...
public class EncryptionService {
	...
	private static final int NIVBITS = 128; // CBC Initialization Vector
											// (same as cipher block size)
											// [16 bytes]
	private static final int NKEYBITS = 256;
	...
	public EncryptionService() {
		// Add BC provider if it is not installed yet.
		Security.addProvider(new BouncyCastleProvider());
	}

	public String encode(String dec) throws GeneralSecurityException {
		// Generate 128 bits of random data for use as the IV. It is important
		// to use a different IV for
		// each encrypted block of text, to ensure that the same string
		// encrypted by two different people
		// does not give the same encrypted text string - that leads to obvious
		// attack possibilities. Note
		// however that the IV does not need to be kept secret; it is a little
		// bit like a 'salt' for a
		// password, which improves security even when the salt is stored in
		// plaintext in a database or
		// prefixed to the encrypted file.
		byte[] ivData = new byte[NIVBITS / 8];
		// Note: no seed here, ie these values are truly random
	...
	private KeyParameter getAESKey() throws GeneralSecurityException {
		...
		PBEKeySpec pbeKeySpec = new PBEKeySpec(password.toCharArray(), salt.getBytes(UTF8), NKEYITERATIONS, NKEYBITS);
