...
public class DashboardService {
	...
	public List<Integer> getMonthlyCumulativeMigrated(String startDate, String endDate) {
		...
		String sql = "select\n" + 
				...
				"      ) d1\n" + 
				"  ) d2\n" + 
				"  left outer join c_agent a on (DATE_FORMAT(a.last_installation_date, '%m-%Y') = DATE_FORMAT(d2.m1, '%m-%Y') OR DATEDIFF(a.last_installation_date, d2.m1) <= 0)\n" + 
				"where d2.m1 <= STR_TO_DATE(:endDate, '%m-%Y')\n" + 
				"group by d2.m1\n" + 
				"order by d2.m1";
		// @formatter:on
	...
	public List<Integer> getMonthlyCumulativePlanned(String startDate, String endDate) {
		...
		String sql = "select\n" + 
				...
				"                      inner join c_agent_tag t ON (ta.tag_id = t.id)\n" + 
				"                    group by ta.agent_id\n" + 
				"                  ) d3 on (DATE_FORMAT(d3.planned_migration_date, '%m-%Y') = DATE_FORMAT(d2.m1, '%m-%Y') OR DATEDIFF(d3.planned_migration_date, d2.m1) <= 0)\n" + 
				"where d2.m1 <= STR_TO_DATE(:endDate, '%m-%Y')\n" + 
				"group by d2.m1\n" + 
				"order by d2.m1";
		// @formatter:on
